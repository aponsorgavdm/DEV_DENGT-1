...
$(document).ready(function(){
	...
	$('#SwitchMail').on('checked', function (event) {
		document.getElementById("SuiviMail").value = 'Non';
	});
	$('#SwitchMail').on('unchecked', function (event) {
		document.getElementById("SuiviMail").value = 'Oui';
	});
	...
	$("#ValidOPI").on('click', function() {
		...
		if ((document.getElementById("input_ExOPI").value != '') && (document.getElementById("input_TypOPI").value != '') && (document.getElementById("input_NumOPI").value != '')) {
			if ((document.getElementById("input_ExOPI").value.length == 4) && (document.getElementById("input_NumOPI").value.length == 4)) {
				...
				var Num_OPI = document.getElementById("input_NumOPI").value;
	...
	//--------------------- SelectType_CTR -------------------
	//////////////////////////////////////////////////
	$("select#SelectType_CTR").jqxDropDownList({ width: 90, height: 22, autoDropDownHeight: true });
		...
		if (lign != null) {
			...
			$.ajax({
				...
				success : function(code_html_retour, statut){
					...
					$("select#SelectNum_OPA").jqxDropDownList({ width: 140, height: 22, dropDownHeight: 150, dropDownWidth: 500, dropDownHorizontalAlignment: 'left' });
					// ci dessous re generer le on select
								...
								$.ajax({
									...
									success : function(code_html_retour, statut){
										...
										$(".SelectMip"	).click(function(){
											...
											$('#DivMipDetail').replaceWith('<div id="DivMipDetail" style="font-size: 14px;color: #666666;display:none;">DivMipDetail '+recupMip+'</div>');
											$("#DivMipDetail").show();
											...
											$("#DivDepasMip").show();
											$('#Textejustifdepas').replaceWith('<div id="Textejustifdepas">Textejustifdepas '+recupMip+' dans DivDepasMip</div>');
										}).css({'cursor':'pointer'});
									},
								});
//								
		...
		if (lign != null) {
			...
			$.ajax({
				...
				success : function(code_html_retour, statut){
					...
					$("select#SelectNum_OPA_IMP").jqxDropDownList({ width: 500, height: 22, dropDownHeight: 150, dropDownWidth: 500, dropDownHorizontalAlignment: 'left' });
					...
					if (v_ex_OPA != '?') {
						...
						success : function(code_html_retour, statut){
							...
							$(".SelectMip"	).click(function(){
								...
								$("#DivDepasMip").show();
								$("#expandListMip").jqxExpander({ expanded: false });
								$('#Textejustifdepas').replaceWith('<div id="Textejustifdepas">'+recupMip+'</div>');
							}).css({'cursor':'pointer'});
						},
					});
					} else { // if (v_ex_OPA != '?') {
	...
	$("select#SelectIbFonc").jqxDropDownList({ width: 140, height: 22, dropDownHeight: 150, dropDownWidth: 500, dropDownHorizontalAlignment: 'left' });
	$("select#SelectNum_OPA").jqxDropDownList({ width: 140, height: 22, dropDownHeight: 150, dropDownWidth: 500, dropDownHorizontalAlignment: 'left' });
	$("select#SelectNum_OPA_IMP").jqxDropDownList({ width: 140, height: 22, dropDownHeight: 150, dropDownWidth: 500, dropDownHorizontalAlignment: 'left' });
		
	$("#expandMarEtTiers").jqxExpander({ width: '1020px'});
	$("#expandMtEtDatess").jqxExpander({ width: '1020px'});
	$("#expandObjEtLieu").jqxExpander({ width: '1020px', expanded: false });
	$("#expandTextLibelle").jqxExpander({ width: '1000px', expanded: false });
	$("#expandCompta").jqxExpander({ width: '1020px', expanded: false });
	...
	//--------------------- radioTrx -------------------
	//////////////////////////////////////////////////
	$("#radioTrx1").jqxRadioButton({ groupName: '1', width: '300px', height: 25, checked: true });
	$("#radioTrx1").on('change', function (event) {
		var checked = event.args.checked;
		if (checked) {
			$("#input_EtudTrx").val('Trx');
		}
	});
	$("#radioTrx2").jqxRadioButton({ groupName: '1', width: '300px', height: 25 });
	$("#radioTrx2").on('change', function (event) {
		var checked = event.args.checked;
		if (checked) {
			$("#input_EtudTrx").val('ETrx');
		}
	});
	$("#radioTrx3").jqxRadioButton({ groupName: '1', width: '300px', height: 25 });
	$("#radioTrx3").on('change', function (event) {
		var checked = event.args.checked;
		if (checked) {
			$("#input_EtudTrx").val('ENTrx');
		}
	});
	//////////////////////////////////////////////////
	//--------------------- FIN radioTrx -------------------
	...
	//--------------------- radioInvest -------------------
	//////////////////////////////////////////////////
	$("#radioInvest1").jqxRadioButton({ groupName: '2', width: '100px', height: 25, checked: true });
	...
	$("#radioInvest2").jqxRadioButton({ groupName: '2', width: '200px', height: 25 });
	...
	$("#radioInvest3").jqxRadioButton({ groupName: '2', width: '100px', height: 25 });
