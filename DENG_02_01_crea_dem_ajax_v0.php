	...
	if ($pdo_total_sql == 1) { // le resultat de la recherche de l'OPI donne 1 seul resultat 
		...
		foreach ($pdo_sqlOPI_ib  as $key_sqlOPI_ib  => $pdo_lignes_sqlOPI_ib ) {
			...
			$sqlOPI_libib = "
				...
				and 
				H2_000_Id_Ib_Lib = concat('N_','".$pdo_lignes_sql['F_006_Budget']."','_',H_027_Ib_Nature) 
...
if ((isset($_POST['v_ajax'])) and ($_POST['v_ajax'] == "SelectIbNat") and ($_POST['v_H_027'] != "") and ($_POST['v_H_006'] != "")) {
    ...
    <select name="SelectIbFonc" id="SelectIbFonc">
        ...
        if ($pdo_total_sql > 0) {
            foreach ($pdo_sql  as $key_sql  => $pdo_lignes_sql ) {
                echo '<option value="'.$pdo_lignes_sql['H_028_Ib_Fonction'].'">'.$pdo_lignes_sql['H_028_Ib_Fonction'].'-'.utf8_encode($pdo_lignes_sql['H2_104_Libelle']).'</option>';
            }
        }
        ?>
    </select>
...
if ((isset($_POST['v_ajax'])) and ($_POST['v_ajax'] == "SelectNumOPA") and ($_POST['v_F5_000'] != "")) {
	if ((isset($_POST['v_input_NumMip'])) and ($_POST['v_input_NumMip'] != "")) { // on arrive via n° mip direct $_POST['v_F5_000'] = exercice du MIP
		$sql = "select * from F5_OPE_MIP where F5_131_Id_Niv_1 = '".$_POST['v_F5_000']."' and F5_135_Id_Niv_3 like '%".$_POST['v_input_NumMip']."' and F5_001_Date_Maj = '".date('Y-m-d')."' order by F5_000_id_Ope, F5_131_Id_Niv_1, F5_133_Id_Niv_2, F5_135_Id_Niv_3";
			#echo $sql."<br>";
		...
		$sql = "select * 
			...
			and 
			F5_001_Date_Maj = '".date('Y-m-d')."' 
			and 
			(F5_133_Id_Niv_2 = '".($_SESSION['VG_droits_DENGT_Ardt']*1)."' OR F5_133_Id_Niv_2 = 'NL') 
	...
	if ($pdo_total_sql > 0) {
		  ...
		  <div id="DivMip" style="font-size: 14px;color: #666666;display:none;">  	
			  ...
			  <table width="1000" border="1" align="center" cellpadding="0" cellspacing="0" style="font-size: 8px;color: #333333;">
                ...
                ?>
                <tr>
                    <td scope="row" style="text-align: center"><div id="<? echo $pdo_lignes_MIP['F5_131_Id_Niv_1']."_".$pdo_lignes_MIP['F5_133_Id_Niv_2']."_".$pdo_lignes_MIP['F5_135_Id_Niv_3']."$".$v_mt_dispo; ?>" class="SelectMip">Choisir</div></td>
...
if ((isset($_POST['v_ajax'])) and ($_POST['v_ajax'] == "SelectCE_CTR") and ($_POST['v_item'] != "")) {
	$sql = "
			...
			and 
			D3_117_Ardt like'%".$_SESSION['VG_droits_DENGT_Ardt']."%' 
			and 
